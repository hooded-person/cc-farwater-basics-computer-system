...
local loop = require "lib.taskmaster" ()
...
local function addProcess(name, func, options)
    ...
    local process = {
        name = name,
        entrypoint = options.entrypoint or tostring(func),
    ...
    if not options.windowless then
    ...
    process.task = loop:addTask(
        keepAlive(process, function()
            ...
            func(process.data_folder, process.shared)
            if process.win then
...
local function addProcessFile(name, entrypoint, options)
    ...
    return addProcess(
        name,
        function(data_folder, shared)
            shell.execute(entrypoint, data_folder, textutils.serialise(shared, { compact = true }))
        end,
        options
    )
end
...
for index, data in pairs(daemons) do
    ...
    local options = { -- all options should be listed here, but those asigned nil are reserved for internal use
        windowless = data.windowless,
        eventBlacklist = nil,
        confirmKill = data.confirmKill,
        hooks = nil,
    }

    local ok, err = addProcessFile(name, entrypoint, options)
    if not ok then
...
local function updateProcessList()
    ...
    for name, process in pairs(processes) do
        ...
        local now = os.epoch("utc") / 1000
        local fg = process.win ~= nil and "44440" or "ffff0"
        local bg = "fffff"
...
local function daemonUIClicks()
    ...
    while true do
        drawMainBar(
            openedProcessWindow == nil and "Daemon UI" or
            openedProcessWindow .. " (" .. processes[openedProcessWindow].entrypoint .. ")",
        ...
        if event == "mouse_click" then
            ...
            elseif y > 1 and processList[y - 1] ~= nil then -- in the process list, on a valid process
                local process_name = processList[y - 1]     -- Kill button
                ---@class Process
                local process = processes[process_name]
                if x >= w - 8 and x <= w - 4 and process.win ~= nil then -- View button
